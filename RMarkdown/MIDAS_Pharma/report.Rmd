    ...
    output$bar_plot <- renderPlotly({
      ...
      bar_plot <-
        ...
        labs(x = "", y = ifelse(input$metric == "ddd", "DDD", "SU"), 
             caption = paste0(round((sum(DATA$count_amr, na.rm = T)/length(unique(DATA$corporation)))*100, 2), "% AMR alliance only; ", 
                              round((sum(DATA$count_ifpma, na.rm = T)/length(unique(DATA$corporation)))*100, 2), "% IFPMA only; ",
                              round((sum(DATA$count_both, na.rm = T)/length(unique(DATA$corporation)))*100, 2), "% both; ", 
                              round((sum(DATA$count_neither, na.rm = T)/length(unique(DATA$corporation)))*100, 2), "% neither."
                             ), fill = NULL) +
